...
class TrainDataset(BaseDataset):
    ...
    def __getitem__(self, index):
        path_to_image = self.csv_file.iloc[index]['Image']
        path_to_mask = self.csv_file.iloc[index]['Mask']
        frame = self.csv_file.iloc[index]['Frame']

        image = self.read_tiff_file_frame(os.path.join(self.path, path_to_image), frame)
        mask = self.encode_segmentation_map(self.read_tiff_file_frame(
            os.path.join(self.path, path_to_mask), frame))
    ...
    def encode_segmentation_map(self, image):
        n_classes = 2
