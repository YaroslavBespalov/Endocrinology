...
class Factory:
    ...
    def make_model(self):
        ...
        # print(self.params['model_params'])
        ...
        if 'weights' not in self.params or self.params['weights'] is None:
            ...
            return model
        ##############
        elif model_name == 'cascade' and isinstance(self.params['weights'], str):
            model.stage1.load_state_dict(torch.load(self.params['weights'])['state_dict'])
            return model
        ##############
        else:
